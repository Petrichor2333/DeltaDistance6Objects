...
void ValueOfV(ObjectDDFuncInfo* obj1, ObjectDDFuncInfo* obj2, double* x, double* val, double R[][3], double* p) {
...
void GradOfV(ObjectDDFuncInfo* obj1, ObjectDDFuncInfo* obj2, double* x, double* Grad, double R[][3], double* p) {
	double x_in_2[3] = { 0,0,0 };
	for (int i = 0;i < 3;i++) {
		for (int j = 0;j < 3;j++) {
			x_in_2[i] += R[i][j] * x[j];
...
void HessOfV(ObjectDDFuncInfo* obj1, ObjectDDFuncInfo* obj2, double* x, double* Hess, double R[][3], double* p) {
	double x_in_2[3] = { 0,0,0 };
	for (int i = 0;i < 3;i++) {
		for (int j = 0;j < 3;j++) {
			x_in_2[i] += R[i][j] * x[j];
...
bool SolveNewtonStep(double* H, double* g, double* x) {
	...
	if (CholeskySolveNewtonStep(H, g, x)) {
		return 1;
	}	
...
bool SolveForCollision(ObjectDDFuncInfo* obj1, ObjectDDFuncInfo* obj2, double* xInit, ResultsOfSolvingMinimum * Result, double R1[][3], double* p1, double R2[][3], double* p2) {
	...
	while (GradNorm > ERROR_GRAD && icount < 51)  {	
		...
		if (!SolveNewtonStep(Hess, Grad, q)) {//Solve the Newton step p=-Hess^{+}Grad
			Result->if_success = 0; 
			return 0; 
		}	
		...
		double V_al;
		double x_al[3];
		x_al[0] = x[0] + alpha * q[0], x_al[1] = x[1] + alpha * q[1], x_al[2] = x[2] + alpha * q[2];
		ValueOfV(obj1, obj2, x_al, &V_al, R, p);
		double  alpha_b = ro * alpha;
		double V_al_b;
		double x_al_b[3];
		x_al_b[0] = x[0] + alpha_b * q[0], x_al_b[1] = x[1] + alpha_b * q[1], x_al_b[2] = x[2] + alpha_b * q[2];
		ValueOfV(obj1, obj2, x_al_b, &V_al_b, R, p);
		while ( V_al-V_al_b > 0 && alpha > 0.1) {
			alpha = alpha_b;
			x_al[0] = x_al_b[0], x_al[1] = x_al_b[1], x_al[2] = x_al_b[2];
			V_al = V_al_b;
			alpha_b *= ro;
			x_al_b[0] = x[0] + alpha_b * q[0], x_al_b[1] = x[1] + alpha_b * q[1], x_al_b[2] = x[2] + alpha_b * q[2];
			ValueOfV(obj1, obj2, x_al_b, &V_al_b, R, p);
		}
		x[0] = x_al[0], x[1] = x_al[1], x[2] = x_al[2];
	...
	if (icount > 49) {
		...
		cout << "Failed V: --------------" << endl;
		double V_fail;
		ValueOfV(obj1, obj2, x, &V_fail, R, p);
		cout << V_fail << endl;
		cout << endl;
	...
	ValueOfV(obj1, obj2, x, &(Result->V_min), R, p);
	...
	Result->ConvPoint[0] = xInit[0], Result->ConvPoint[1] = xInit[1], Result->ConvPoint[2] = xInit[2];
